...
for d in tqdm(all_dim):
    ...
    for r in range(rep):
        ...
        Sigma = np.diag(np.random.random(dim_TS) / 5)
            ...
            results.append(
                {
                    ...
                    "error_sigma": distance_riemann(
                        Sigma, np.diag(res_optim.point[2] ** 2)
                    ),
                }
            )

results_dataframe = pd.DataFrame(results)
results_dataframe.to_pickle("results_vs_number_points_diagonal_10.pkl")
